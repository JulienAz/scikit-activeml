...
if __name__ == '__main__':
    ...
    df_merged = pd.merge(df_clustering, df_acc, on=[X1, X2, BUDGET, CLU_TIMEWINDOW, N_CLUSTER])

    sb.set_theme()

    g = sb.relplot(data=df_merged, x=X1, y=X2, kind='scatter', palette="deep", edgecolor='White', linewidth=0.5, hue=GT, col=N_CLUSTER, col_wrap=3)
    df_labeled = df_merged.loc[~np.isnan(df_merged['Label_x'])]

    axes = g.axes

    center_radi = df_clustering[[C1, C2, RADIUS, BUDGET, N_CLUSTER]].drop_duplicates().to_numpy()
